...
Four earlier revisions produced materially different grouped spike scores. They remain under `evidence/development/` as non-canonical development evidence; only `evidence/final/` is the Phase 2 reference used by later stages. Current AR contracts live in `src/neural_bridge/again/`; the superseded phase entrypoint is not duplicated.
