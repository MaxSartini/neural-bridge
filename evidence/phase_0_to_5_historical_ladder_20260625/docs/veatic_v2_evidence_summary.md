...
Current status: VEATIC-124 v2 established the original controlled future arousal event/spike-ranking signal. The current repo claim pairs that foundation with the later AGAIN Phase 5.5 blocked binary confirmation and repaired grouped-video compatibility evidence. This historical bundle should be read as the VEATIC foundation plus early AGAIN ladder snapshot, not as the final current framing.

VEATIC-124 v2 supports specific Neural Bridge hypotheses for arousal event/spike ranking using a video-dominant cortical/TRIBE cache. It shows that visual/video-driven cortical/TRIBE PCA feature modes can improve future arousal spike/event ranking over autoregressive, shuffled, random, timestamp, and video/time controls under blocked and grouped-video validation.
...
- Dense AGAIN V-JEPA 2.1 / TRIBE v2 data generation later completed on H100 for all `995` videos at `2Hz`, `256px`, float16. The current Phase 5.5 claim now uses AGAIN as the scaled confirmation ladder: bounded strict forward-time future-event ranking is proven on `future_arousal_max_delta_rows_4_10_train_q90` with `short_temporal_conv_residual`, while continuous exact forecasting and broad all-target/all-dataset temporal prediction remain open.
