    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <div class="neuro-prior-panel">
            <label class="switch-control neuro-switch">
              <input type="checkbox" v-model="enableNeuroPriors">
              <span class="switch-track"></span>
              <span class="switch-label">Enable neuro-prior modelling</span>
            </label>
            <p class="description compact">
              Estimates population-level salience, threat, reward, arousal, uncertainty, memory relevance, approach/avoidance, polarisation, and virality priors, then uses them to bias agent behaviour.
            </p>
            <div v-if="enableNeuroPriors" class="neuro-form">
              <select v-model="stimulusType" class="neuro-select">
                <option value="text">Text</option>
                <option value="audio">Audio (coming soon)</option>
              ...
              <textarea
                ...
                rows="5"
                placeholder="Paste the speech, announcement, post, campaign message, or market communication to calibrate..."
              ></textarea>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            <!-- Time Configuration -->
            <div v-if="neuroPrior" class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Neuro-Prior Calibration</span>
                <span class="config-block-badge">{{ neuroPrior.mode || 'proxy' }}</span>
              </div>
              <div class="neuro-score-grid">
                <div v-for="item in neuroScoreItems" :key="item.key" class="neuro-score">
                  <span class="neuro-score-label">{{ item.label }}</span>
                  <span class="neuro-score-value">{{ formatScore(neuroPrior[item.key]) }}</span>
                </div>
              </div>
              <p v-if="neuroPrior.behavioural_prior_summary" class="reasoning-text">
                {{ neuroPrior.behavioural_prior_summary }}
              </p>
            </div>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">Neural Bridge Automatically plan and infer reality <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours，Each round represents reality <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes time elapsed</span>
              </div>
...
const enableNeuroPriors = ref(true)
const stimulusText = ref('')
const stimulusType = ref('text')
const neuroPrior = ref(null)
...
const neuroScoreItems = [
  { key: 'salience_score', label: 'Salience' },
  { key: 'threat_score', label: 'Threat' },
  { key: 'reward_score', label: 'Reward' },
  { key: 'arousal_score', label: 'Arousal' },
  { key: 'uncertainty_score', label: 'Uncertainty' },
  { key: 'approach_bias', label: 'Approach' },
  { key: 'avoidance_bias', label: 'Avoidance' },
  { key: 'polarisation_risk', label: 'Polarisation' },
  { key: 'virality_pressure', label: 'Virality' }
]
...
const startPrepareSimulation = async () => {
  ...
  try {
    const res = await prepareSimulation({
      ...
      enable_neuro_priors: enableNeuroPriors.value,
      stimulus_text: stimulusText.value,
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        neuroPrior.value = data.config.neuro_prior && Object.keys(data.config.neuro_prior).length
          ? data.config.neuro_prior
          : neuroPrior.value
        await fetchNeuroPrior()
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        neuroPrior.value = res.data.config.neuro_prior && Object.keys(res.data.config.neuro_prior).length
          ? res.data.config.neuro_prior
          : neuroPrior.value
        await fetchNeuroPrior()
...
const fetchNeuroPrior = async () => {
  if (!props.simulationId || neuroPrior.value) return
  try {
    const res = await getNeuroPrior(props.simulationId)
    if (res.success && res.data?.neuro_prior) {
      neuroPrior.value = res.data.neuro_prior
    }
  } catch (err) {
    // Neuro-prior is optional; absence should not interrupt setup display.
  }
}
...
onMounted(() => {
  stimulusText.value = props.projectData?.simulation_requirement || ''
