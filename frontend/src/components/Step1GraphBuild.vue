<template>
  <div class="workbench-panel">
    <div class="scroll-container">
      ...
      <!-- Screenshot Ingestion (optional) -->
      <div class="step-card" :class="{ 'active': currentPhase >= 1 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">02+</span>
            <span class="step-title">Comment Screenshots <span class="optional">(optional)</span></span>
          </div>
          <div class="step-status">
            <span v-if="screenshotResult" class="badge success">Done</span>
            <span v-else-if="uploadingScreenshots" class="badge processing">Extracting</span>
            <span v-else class="badge pending">Idle</span>
          </div>
        </div>
        <div class="card-content">
          <p class="api-note">POST /api/scrape/screenshots</p>
          <p class="description">
            Upload screenshots from X, Reddit, Facebook, Instagram, YouTube, or any comment
            section to seed agents with real opinions.
          </p>
          ...
          <input
            ...
            multiple
            @change="onScreenshotsSelected"
            style="display:none"
          />
          <button
            class="action-btn"
            :disabled="!projectData?.graph_id || uploadingScreenshots"
            @click="$refs.screenshotInput.click()"
          >
            Select Screenshots
          </button>

          <div v-if="screenshotPreviews.length" class="preview-grid">
            <img
              v-for="(url, i) in screenshotPreviews"
              :key="i"
              :src="url"
          ...
          <input
            v-if="screenshotFiles.length"
            v-model="screenshotTopic"
            type="text"
            placeholder="What topic are these about? e.g. Eskom tariff increase"
            class="topic-input"
          />

          <button
            v-if="screenshotFiles.length && !screenshotResult"
            class="action-btn"
            :disabled="uploadingScreenshots"
            @click="uploadScreenshots"
          >
            <span v-if="uploadingScreenshots" class="spinner-sm"></span>
            {{ uploadingScreenshots ? 'Extracting...' : 'Extract Comments' }}
          </button>

          <div v-if="screenshotResult" class="screenshot-badge">
            {{ screenshotResult.total_comments }} comments extracted from
            {{ screenshotResult.total_images }} screenshot(s)
          </div>
          <div v-if="screenshotError" class="screenshot-error">{{ screenshotError }}</div>
        </div>
      </div>
...
const screenshotTopic = ref('')
const screenshotResult = ref(null)
const screenshotError = ref('')
const uploadingScreenshots = ref(false)

const onScreenshotsSelected = (event) => {
  screenshotFiles.value = Array.from(event.target.files || [])
  screenshotPreviews.value = screenshotFiles.value.map((f) => URL.createObjectURL(f))
...
const uploadScreenshots = async () => {
  if (!props.projectData?.graph_id) {
    screenshotError.value = 'Graph not ready yet'
    return
  }
  ...
  try {
    ...
    formData.append('graph_id', props.projectData.graph_id)
    if (props.projectData?.project_id) {
      formData.append('project_id', props.projectData.project_id)
    }
    formData.append('topic_context', screenshotTopic.value || '')
    screenshotFiles.value.forEach((f) => formData.append('files', f))

    const res = await axios.post('/api/scrape/screenshots', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    if (res.data?.success) {
      screenshotResult.value = res.data
    } else {
      screenshotError.value = res.data?.error || 'Unknown error'
    }
  } catch (e) {
    console.warn('Screenshot extraction failed:', e)
    screenshotError.value = e.message || 'Upload failed'
  } finally {
...
const handleEnterEnvSetup = async () => {
  ...
  try {
    const res = await createSimulation({
      ...
      enable_reddit: true,
      enable_neuro_priors: true,
      stimulus_text: props.projectData.simulation_requirement || '',
      stimulus_type: 'text'
    })
