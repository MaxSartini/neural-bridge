<template>
  <div class="home-container">
    ...
    <nav class="navbar" :style="s.navbar">
      <div class="nav-brand" :style="s.navBrand">NEURAL BRIDGE</div>
      <div class="nav-links" :style="s.navLinks"></div>
    </nav>
      ...
      <section class="hero-section" :style="s.heroSection">
        ...
        <div class="hero-right" :style="s.heroRight">
          <div class="logo-container" :style="s.logoContainer">
            <img src="../assets/logo/neural-bridge-logo-left.jpeg" alt="Neural Bridge Logo" :style="s.heroLogo" />
          </div>
          ...
          <div class="console-box" :style="s.consoleBox">
            <div :style="s.consoleSection">
              <div class="console-header" :style="s.consoleHeader">
                ...
                <span>
                  Supported: PDF, MD, TXT + Screenshots (PNG, JPG)
              ...
              >
                <input ref="fileInput" type="file" multiple accept=".pdf,.md,.txt,.csv,.xlsx,.xls,.png,.jpg,.jpeg,.webp" @change="handleFileSelect" style="display: none" :disabled="loading" />
...
const s = reactive({
  ...
  uploadZone: { border: '1px dashed #CCC', height: '420px', overflowY: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: '#FAFAFA' },
...
const canSubmit = computed(() => {
  // Prompt required. Files optional — if none, we synthesize a doc from the prompt.
...
const addFiles = (newFiles) => {
  const allowed = ['.pdf', '.md', '.txt', '.csv', '.xlsx', '.xls', '.png', '.jpg', '.jpeg', '.webp']
