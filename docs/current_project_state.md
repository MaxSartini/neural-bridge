...
The approved bounded 420-row selected-head confirmation is complete. The subsequent one-seed, 16-trial Optuna pilot on MLX reproduced the original exactly (`0.2697372519`) and improved held-out PR-AUC to `0.2718557352`, with `+0.0081646445` over frozen AR and the best matched control. This is promising exploratory evidence only. The next bounded experiment, when explicitly started, is a single locked-winner 10-seed blocked confirmation against existing original rows and the same matched controls; do not optimize separately per seed or begin grouped/broad/continuous work. The historical 504 development matrix remains out of scope.
