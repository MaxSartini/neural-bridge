...
The approved bounded 420-row selected-head confirmation remains canonical. The one-seed Optuna winner failed aggregate 10-seed improvement. A later 24-trial robust multi-seed Optuna Stage A used only inner train/validation evidence and also failed its reserved five-seed robust-objective gate, so the planned 720-row blocked/grouped campaign was not run. The canonical original recipe remains best supported within this hyperparameter family. Seed `20260627` had an unusually favorable canonical-original peak, but was retained in every primary verdict. Do not optimize against viewed held-out scores or claim 720/504. Any next development should test a genuinely new bounded modeling hypothesis.
