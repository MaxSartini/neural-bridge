...
The approved bounded 420-row selected-head confirmation remains canonical. A robust Optuna campaign reached Stage B after a fresh-seed inner-validation rescue, then failed after `120/120` blocked rows: trial 4 was slightly higher on all-seed mean/median and 19.4% less variable, but lost the untouched fresh-five mean/consistency gate. It still beat AR and controls in `15/15`; this is a strong alternative, not a promoted replacement. Grouped Stage C was not run, so do not claim 720. A subsequent fixed 50/50 original/Trial-4 pilot failed because its tiny `+0.0001625` gain over Trial 4 did not reduce variance. The next bounded hypothesis is true averaging across independently trained checkpoints, not another within-seed configuration blend.
