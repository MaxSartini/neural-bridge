...
The approved bounded 420-row selected-head confirmation remains canonical. The one-seed Optuna pilot was promising, but its preregistered locked-winner 10-seed confirmation failed aggregate improvement: tuned minus original was `-0.0011081356` on all seeds and `-0.0014666488` on the nine follow-up seeds, despite `7/10` positive paired seeds and retained controlled wins over AR/controls. Seed `20260627` had an unusually favorable canonical-original training peak; a post-hoc extended convergence run ruled out tuned under-training but cannot remove the seed or change the verdict. The next bounded development step is a multi-seed inner-validation Optuna objective with a distinct reserved evaluation protocol. Do not optimize against viewed held-out scores or begin grouped/broad/continuous work. The historical 504 development matrix remains out of scope.
