...
Stage 0 of `zero_label_video_only_deployment_bridge_pilot_preregistration.md` is complete and passed all target/split/feature/cold-start contracts on the real `995`-video substrate. Its evidence snapshot is `../evidence/zero_label_video_only_deployment_stage0_20260714/`; no model training or held-out scoring occurred, and Stage A remains unauthorized. The later `96/96` development screen and separately authorized `140/140` locked confirmation require continuous Spearman, top-5% lift, and event PR-AUC jointly. A pass retains at least half of the AR-assisted teacher's incremental gain over the matched no-video zero-label anchor on each endpoint; it does not unrealistically require a cold-start model to equal the teacher's absolute Phase 7 score.
