...
Stage 0 of the bounded zero-label protocol has now passed on the real `995`-video / `243,575`-row substrate. Canonical evidence is `evidence/zero_label_video_only_deployment_stage0_20260714/`: exact future matrices `96/96` Stage A and `140/140` Stage B, raw-target identity digest `446906dff30be33f204de0f973207975`, development/locked split digests `cf65a766cd827e6201544dd753049cb4` / `ded8bc2bf079fef91ae5c253b9a9ac2e`, and failed contracts `[]`. Stage 0 fit no model, generated no teacher scores, scored no held-out predictions, and did not authorize Stage A.
...
The Stage 0 target-identity audit resolved the Phase 7 name/value hazard by locking the actual `future_arousal_max_delta_rows_4_10` value and matching mask/row/source digests. This is not a retroactive change to the Phase 7 pass. Stage 0 also found that old fold-specific PCA score matrices use incompatible coordinate systems for the new `696/299` split. Later work must keep the frozen cortical substrate and fixed `temporal_mean_2s` / PCA256 recipe but fit each PCA basis inside the applicable outer training pool; nested teacher PCA must be fit only inside its teacher-training partition.
