...
Routine continuity uses the compact fast path: update and validate `docs/handoff/CURRENT_STATE.md`, index that file and `docs/tokless_agent_workflow.md` into Context-Mode, and prove it with a top-3 search query. Do not index the full project or historical evidence tree after ordinary result or documentation changes. Full-corpus indexing is reserved for an explicitly requested structural evidence/archive rebuild.
