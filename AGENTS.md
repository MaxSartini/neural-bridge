...
The next approved development is the staged `docs/phase6_robust_optuna_720_plan.md`: a 24-trial multi-seed inner-validation Optuna study, followed only on gate pass by a 15-seed/8-lane blocked matrix (`120` rows) and five-fold grouped matrix (`600` rows). The full new matrix is `720` rows and remains separate from the canonical 420. Stop fail-closed at Stage A or B; do not optimize against blocked/grouped held-out scores, delete outlier seeds from the primary verdict, restore 504, or begin continuous-model development.
