...
The staged `docs/phase6_robust_optuna_720_plan.md` stopped fail-closed at Stage A. Its 24-trial, five-development-seed Optuna study did not read blocked/grouped held-out scores. Best trial 22 won `4/5` reserved inner-validation seeds, but mean improvement over original was only `+0.0000082124` and the robust objective was worse by `-0.0004650065`; Stage A pass was false. The planned `120` blocked + `600` grouped = `720` rows were not run and must not be claimed.

Post-hoc sensitivity showed that seed `20260627` materially changed development selection: excluding it from the development objective selected trial 4, which then beat original on all `5/5` original reserved inner-validation seeds with `+0.0048272773` robust-objective gain. This does not retroactively pass Stage A. The next approved step is the Stage A2 rescue in `docs/phase6_robust_optuna_720_plan.md`: lock trial 4 unchanged and validate it inner-only on five new seeds `20260635`–`20260639`. Only a prespecified Stage A2 pass authorizes the 720-row campaign; seed `20260627` must remain in later primary matrices as a stress-test.
