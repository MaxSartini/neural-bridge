...
- AGAIN boundary, scout, full-AR-context, and sparse-teacher tooling is implemented. The current tracked AGAIN sparse teacher result is bounded 50-video evidence: PCA128 failed the first pilot, the 500-window small-PCA follow-up looked promising, and the 2000-budget confirmatory run did not confirm promotion against nuisance and matched-random controls.
...
- [x] Run the 2000-budget sparse teacher confirmatory pass; record that it completed 1,948 windows and did not justify broader sparse-teacher scaling.
