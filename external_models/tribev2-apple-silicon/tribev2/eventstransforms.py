...
class ExtractWordsFromAudio(EventsTransform):
    ...
    def _get_transcript_from_audio(wav_filename: Path, language: str) -> pd.DataFrame:
        ...
        transcription_backend = os.environ.get(
            "TRIBE_TRANSCRIPTION_BACKEND", default_backend
        ).lower()
        if transcription_backend == "mlx":
            import mlx_whisper

            mlx_model = os.environ.get(
                "TRIBE_MLX_WHISPER_MODEL", "mlx-community/whisper-small-mlx"
            )
            logger.info("Running mlx-whisper model=%s on Apple Metal", mlx_model)
        ...
        elif transcription_backend == "whisperx":
            # whisperx transcription uses CTranslate2, which supports CUDA or
            # CPU but not Apple MPS. Use MLX above for Apple GPU acceleration.
            ...
            whisper_model = os.environ.get("TRIBE_WHISPERX_MODEL", "small")
            batch_size = os.environ.get("TRIBE_WHISPERX_BATCH_SIZE", "4")
            logger.info("Running whisperx model=%s device=%s", whisper_model, device)
            ...
            raise ValueError(
                f"Unsupported TRIBE_TRANSCRIPTION_BACKEND={transcription_backend!r}; "
                "expected 'mlx' or 'whisperx'."
            )
