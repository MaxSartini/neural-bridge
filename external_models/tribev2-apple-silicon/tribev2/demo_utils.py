...
class TextToEvents(pydantic.BaseModel):
    ...
    def get_events(self) -> pd.DataFrame:
        if os.environ.get("TRIBE_TEXT_EVENTS_DIRECT", "true").lower() == "true":
            words = re.findall(r"\b[\w'-]+\b", self.text)
            if not words:
                raise ValueError("No words found in text stimulus.")
            rows = []
            cursor = 0.0
            for idx, word in enumerate(words):
                duration = max(0.18, min(0.7, 0.08 * len(word)))
                rows.append(
                    {
                        "type": "Word",
                        "text": word,
            ...
            events["context"] = events["sentence"].fillna(self.text)
            return events
...
class TribeModel(TribeExperiment):
    ...
    def from_pretrained(
        ...
        if device == "cpu":
            # Keep all extractor devices on CPU only when explicitly required.
