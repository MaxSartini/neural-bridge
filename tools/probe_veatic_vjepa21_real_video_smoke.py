...
def main() -> int:
    ...
    output_root = ROOT / "outputs" / f"veatic_vjepa21_real_video_smoke_{args.timestamp}"
    external_output_root = external_root / "benchmarks" / "veatic" / f"vjepa21_mlx_real_video_smoke_{args.timestamp}"
    ...
    write_csv(output_root / "veatic_vjepa21_real_video_smoke.csv", [smoke_row])
    ...
    report = "\n".join([
        "# VEATIC V-JEPA 2.1 MLX Real-Video Smoke",
        "",
        "## Verdict",
        ...
        "- vjepa21_claim_made: `false`",
        "",
        "No performance claims are made from this one-video smoke.",
        "",
    ])
    (output_root / "veatic_vjepa21_real_video_smoke_report.md").write_text(report, encoding="utf-8")
