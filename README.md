...
Latest results on `2026-07-14`: the fully provisioned backend passed `113` tests, including the real-package research-tooling and selected-head Optuna pilot contracts. The default `npm run verify` environment passed `105` tests and skipped one optional research-tooling module; the repository readiness audit, strict-benchmark dry run, and frontend production build passed.
...
The approved [full bounded 420-row selected-head confirmation](docs/phase5_selected_head_420_confirmation_plan.md) is complete and promoted. A bounded [one-seed Optuna pilot](docs/phase6_optuna_selected_head_pilot_plan.md) then reproduced the exact original held-out PR-AUC (`0.2697372519`) and improved it to `0.2718557352`, with `+0.0081646445` over frozen AR and the best matched control. This justifies only a locked-winner 10-seed blocked confirmation; it is not yet a promoted model improvement. The historical literal 504 matrix remains obsolete. Exact continuous-value forecasting, blocked continuous generalization, and broad all-target/all-dataset temporal prediction remain open; grouped continuous future-movement ranking/lift remains a separate controlled positive result.
