...
The approved [full bounded 420-row selected-head confirmation](docs/phase5_selected_head_420_confirmation_plan.md) remains canonical. The promising [one-seed Optuna pilot](docs/phase6_optuna_selected_head_pilot_plan.md) did not promote: its preregistered [locked 10-seed confirmation](docs/phase6_optuna_locked_10seed_confirmation_plan.md) beat the original in `7/10` seeds but lost on mean PR-AUC (`-0.0011081356`), dominated by an unusually favorable original seed-`20260627` checkpoint. Tuned still beat frozen AR and matched controls, so the model remains valid but not better than the canonical original. Any next Optuna experiment should use an aggregate multi-seed inner-validation objective and a distinct reserved evaluation protocol. The historical 504 matrix remains obsolete; exact continuous-value forecasting, blocked continuous generalization, and broad all-target/all-dataset temporal prediction remain open.
