...
# Get logger
logger = get_logger('neural_bridge.api')
    ...
    try:
        ...
        for file in uploaded_files:
            if file and file.filename and allowed_file(file.filename):
                ...
                # Extract text
                # Use original filename (without extension) as the asset/display name so CSV/Excel
                # assets reflect real tickers instead of random saved-name hashes.
                import os as _os
                _disp = _os.path.splitext(file_info["original_filename"])[0]
                text = FileParser.extract_text(file_info["path"], display_name=_disp)
        ...
        # --- EXPLICIT PANDAS MATHEMATICAL CROSS-CORRELATION ENGINE ---
        # The user mandates that relationships are built upon the truth of the raw data points.
        # This natively extracts correlations and forces them into the LLM context vision.
        try:
            import pandas as pd
            import os
            ...
            for item in project.files:
                path = os.path.join(project.project_dir, item["saved_filename"])
                if path.lower().endswith(".csv"):
                    try:
                        ...
                        # Find primary value column (close, price, or first numeric)
                        val_col = next((c for c in df.columns if c in ['close', 'price', 'value', 'amount']), None)
                        if not val_col:
                            for c in df.columns:
                        ...
                        if date_col and val_col:
                            base_ticker = os.path.splitext(item["original_filename"])[0].upper()
                            clean_df = df[[date_col, val_col]].copy()
                            clean_df.rename(columns={val_col: base_ticker}, inplace=True)
                            ...
                            # Remove duplicate indices (happens in dirty data)
                            clean_df = clean_df[~clean_df.index.duplicated(keep='last')]
                            dfs.append(clean_df)
                    except Exception as e:
                        logger.warning(f"Math Engine skipped {item['original_filename']}: {e}")
            
            if len(dfs) > 1:
                # Merge all datasets into one massive temporal reality
                merged = dfs[0]
                for i in range(1, len(dfs)):
                    merged = merged.join(dfs[i], how='outer')
                
                # Forward/backward fill safely, then compute Pearson
                merged = merged.ffill().bfill()
                corr_matrix = merged.corr(method='pearson')
                
                math_text = "\n=== ABSOLUTE MATHEMATICAL CORRELATIONS (GROUND TRUTH) ===\n"
                math_text += "The following are exact mathematical Pearson correlations calculated directly from the raw data points. You MUST extract these as hard edges between the Assets:\n"
                ...
                for i in range(len(cols)):
                    for j in range(i+1, len(cols)):
                        ...
                        if pd.notna(val):
                            c1, c2 = str(cols[i]), str(cols[j])
                            if val >= 0.70:
                                math_text += f"- STRONG POSITIVE: {c1} natively moves with {c2} (Correlation Score: +{val:.2f}). Extract a 'CORRELATES_WITH' edge between them.\n"
                                has_correlations = True
                            elif val <= -0.60:
                                math_text += f"- STRONG NEGATIVE: {c1} natively moves inversely against {c2} (Correlation Score: {val:.2f}). Extract a 'COMPETES_WITH' or 'ROTATES_INTO' edge between them.\n"
                                has_correlations = True
                            elif 0.30 <= val < 0.70:
                                math_text += f"- WEAK POSITIVE: {c1} has a loose coupling with {c2} (Correlation Score: +{val:.2f}).\n"
                                has_correlations = True
                                
                if has_correlations:
                    all_text = math_text + "\n" + all_text
                    logger.info("Successfully injected Absolute Mathematical Pearson Correlations into extraction text.")
                    
        except Exception as e:
            logger.error(f"Algorithmic Math Engine failed: {e}")

        # Inject the human-typed structural correlation rules directly into the Graph Parser's vision
        all_text = f"=== SIMULATION RULES AND MARKET CORRELATIONS ===\n{simulation_requirement}\n\n" + all_text
        ...
        project.ontology = {
            ...
            # Persist optional ticker→name aliases for downstream NER/graph build
...
def build_graph():
    ...
    try:
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'Neural Bridge Graph')
        ...
        def build_task():
            build_logger = get_logger('neural_bridge.build')
            try:
