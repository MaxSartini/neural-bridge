...
logger = get_logger('neural_bridge.api.simulation')
    ...
    Request (JSON):
        {
            ...
            "graph_id": "neural_bridge_xxxx",    // Optional, if not provided, get from project
            "enable_twitter": false,         // Optional legacy adapter, default false
            "enable_reddit": true            // Optional internal single-channel adapter, default true
        }
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    2. Required files exist for enabled internal adapters plus simulation_config.json
    ...
    state_data_for_files = {}
    if os.path.exists(state_file):
        try:
            import json
            with open(state_file, 'r', encoding='utf-8') as f:
                state_data_for_files = json.load(f)
        except Exception:
            state_data_for_files = {}

    if state_data_for_files.get("enable_reddit", True):
        required_files.append("reddit_profiles.json")
    if state_data_for_files.get("enable_twitter", False):
...
def prepare_simulation():
    ...
    try:
        ...
        # Default to fresh regeneration each prepare so new topics/comments don't reuse stale profiles.
        ...
        neuro_fields_changed = False
        if 'enable_neuro_priors' in data:
            state.enable_neuro_priors = bool(data.get('enable_neuro_priors'))
            neuro_fields_changed = True
        if 'stimulus_text' in data:
            state.stimulus_text = data.get('stimulus_text') or ''
            neuro_fields_changed = True
        if 'stimulus_type' in data:
            state.stimulus_type = data.get('stimulus_type') or 'text'
            neuro_fields_changed = True
        if 'stimulus_media_path' in data:
            state.stimulus_media_path = data.get('stimulus_media_path') or ''
            neuro_fields_changed = True
        if neuro_fields_changed:
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "neural_bridge_xxxx",     // Required
...
def start_simulation():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",          // Required，Simulation ID
            "platform": "neural_bridge",                // Optional: neural_bridge / reddit / twitter / parallel
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"Invalid platform type: {platform}，Optional: neural_bridge/reddit/twitter/parallel"
            }), 400
...
@simulation_bp.route('/<simulation_id>/neuro-prior', methods=['GET'])
def get_neuro_prior(simulation_id: str):
    """Return persisted neuro-prior profile and simulation modifiers."""
    try:
        import json
        ...
        simulation_dir = os.path.join(Config.OASIS_SIMULATION_DATA_DIR, simulation_id)
        prior_path = os.path.join(simulation_dir, "neuro_prior.json")
        modifiers_path = os.path.join(simulation_dir, "neuro_prior_modifiers.json")
        if not os.path.exists(prior_path):
            return jsonify({
                "success": False,
                "error": "Neuro-prior has not been generated for this simulation."
            }), 404
        ...
        return jsonify({
            ...
            "data": {
                "simulation_id": simulation_id,
                "neuro_prior": prior,
                "neuro_modifiers": modifiers,
            }
        })
    except Exception as e:
        logger.error(f"Failed to read neuro-prior: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
