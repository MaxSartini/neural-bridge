...
logger = logging.getLogger('neural_bridge.neo4j_storage')
...
class Neo4jStorage(GraphStorage):
    ...
    def __init__(
        ...
        self._verify_connection_with_legacy_fallback(password_provided=password is not None)
    ...
    def _verify_connection_with_legacy_fallback(self, password_provided: bool) -> None:
        """Verify auth once and fall back for existing databases with the old local password."""
        try:
            self._driver.verify_connectivity()
            return
        except AuthError:
            legacy_password = getattr(Config, "NEO4J_LEGACY_PASSWORD", None)
            if password_provided or not legacy_password or legacy_password == self._password:
                raise
            logger.warning(
                "Neo4j rejected the configured password; retrying with legacy local password."
            )
            self._driver.close()
            self._password = legacy_password
            self._driver = GraphDatabase.driver(
                self._uri, auth=(self._user, self._password)
            )
            self._driver.verify_connectivity()
    ...
    def _extract_data_summary_from_text(self, metric_name: str, text: str) -> Optional[Dict[str, Any]]:
        """
        Extract structured data summary for a metric from a text chunk.
        Looks for lines like: "Metric 'CPI.JO.Close' of asset 'CPI.JO': ... Latest=145.6, Previous=142.3, ..."
        Returns a dict with latest_value, trend, range, etc. for agent context.
        """
        import re
        
        # Normalize metric name for matching (handle both with/without asset prefix)
        base_name = metric_name.split(".")[-1] if "." in metric_name else metric_name
        
        # Search for lines containing this metric
        for line in text.split("\n"):
            if metric_name.lower() in line.lower() or base_name.lower() in line.lower():
                if "Metric" in line or "latest=" in line.lower():
                    try:
                        # Extract numeric values using regex
                        latest_match = re.search(r"Latest[=:]?\s*([\d.e+-]+)", line, re.IGNORECASE)
                        previous_match = re.search(r"Previous[=:]?\s*([\d.e+-]+)", line, re.IGNORECASE)
                        range_match = re.search(r"Range\s+([\d.e+-]+)[–-]([\d.e+-]+)", line, re.IGNORECASE)
                        mean_match = re.search(r"Mean\s+([\d.e+-]+)", line, re.IGNORECASE)
                        trend_match = re.search(r"Trend[:]?\s+(\w+)", line, re.IGNORECASE)
                        pct_match = re.search(r"([+-][\d.]+)%", line)
                        std_match = re.search(r"[Vv]olatility[^:]*[:]?\s*([\d.e+-]+)", line)
                        
                        data_summary = {"metric_name": metric_name}
                        
                        if latest_match:
                            data_summary["latest_value"] = float(latest_match.group(1))
                        if previous_match:
                            data_summary["previous_value"] = float(previous_match.group(1))
                        if range_match:
                            data_summary["min_value"] = float(range_match.group(1))
                            data_summary["max_value"] = float(range_match.group(2))
                        if mean_match:
                            data_summary["mean"] = float(mean_match.group(1))
                        if trend_match:
                            data_summary["trend"] = trend_match.group(1).lower()
                        if pct_match:
                            data_summary["pct_change"] = float(pct_match.group(1))
                        if std_match:
                            data_summary["std_dev"] = float(std_match.group(1))
                        
                        if len(data_summary) > 1:  # More than just the name
                            return data_summary
                    except Exception as e:
                        logger.debug(f"Error extracting data summary: {e}")
    ...
    def add_text(self, graph_id: str, text: str) -> str:
        ...
        # Auto-link Asset to Metric entities if the model did not produce an explicit relation.
        asset_entities = [e for e in entities if e.get("type") == "Asset"]
        metric_entities = [e for e in entities if e.get("type") == "Metric"]
        if asset_entities and metric_entities:
            existing_pairs = {
            ...
            for asset in asset_entities:
                for metric in metric_entities:
                    pair = (asset["name"].lower(), metric["name"].lower())
                    if pair not in existing_pairs:
                        relations.append({
                            "source": asset["name"],
                            "target": metric["name"],
                            "type": "HAS_METRIC",
                            "fact": f"{asset['name']} has metric {metric['name']}"
                        })
        ...
        # Optional ticker → preferred name aliases persisted in ontology
        aliases = {}
        try:
            aliases = ontology.get("aliases", {}) if isinstance(ontology, dict) else {}
        except Exception:
            aliases = {}

        def _apply_alias(name: str, etype: str):
            """Return (display_name, extra_attrs). Applies only for Asset/Index/Company."""
            base = (name or "").replace('.csv', '').strip()
            preferred = aliases.get(base) or aliases.get(base.replace('.JO', ''))
            if preferred and etype in ("Asset", "Index", "Company"):
                display = f"{preferred} ({base})"
                return display, {"ticker": base, "preferred_name": preferred}
            return name, {}
        ...
        with self._driver.session() as session:
            ...
            for idx, entity in enumerate(entities):
                ...
                # Apply alias mapping for nicer display names
                ename, alias_attrs = _apply_alias(original_name, etype)
                if alias_attrs:
                    attrs.update(alias_attrs)
                summary_text = entity_summaries[idx]
                embedding = entity_embeddings[idx] if idx < len(entity_embeddings) else []

                e_uuid = str(uuid.uuid4())
                # Track by both original and display name for relation resolution
                entity_uuid_map[original_name.lower()] = e_uuid
                entity_uuid_map[ename.lower()] = e_uuid

                # For Metric entities, try to extract structured data summary from text chunk
                data_summary_json = None
                if etype == "Metric":
                    try:
                        data_summary_json = self._extract_data_summary_from_text(original_name, text)
                    except Exception as e:
                        logger.debug(f"Could not extract data summary for {original_name}: {e}")

                def _merge_entity(tx, _uuid=e_uuid, _name=ename, _type=etype,
                                  _attrs=attrs, _embedding=embedding,
                                  _summary=summary_text, _data_summary=data_summary_json, _now=now):
                    ...
                    result = tx.run(
                        ...
                        ON CREATE SET
                            ...
                            n.data_summary_json = $data_summary_json,
                        ...
                        ON MATCH SET
                            ...
                            n.attributes_json = $attrs_json,
                            n.data_summary_json = COALESCE($data_summary_json, n.data_summary_json),
                            n.embedding = $embedding
                        ...
                        data_summary_json=json.dumps(_data_summary) if _data_summary else None,
                ...
                entity_uuid_map[original_name.lower()] = actual_uuid
    ...
    def add_commenter_node(
        ...
        """
        MERGE a commenter entity (screenshot-sourced) into the graph.
        Labels applied: :Entity :Commenter :Person.
        Returns the node uuid.
        """
        username = (username or "").strip()
        text = (text or "").strip()
        if not username or not text:
            raise ValueError("username and text are required")

        attrs = dict(attributes or {})
        now = datetime.now(timezone.utc).isoformat()
        new_uuid = str(uuid.uuid4())

        def _fetch_existing(tx):
            r = tx.run(
                "MATCH (n:Entity {graph_id:$gid, name_lower:$name_lower}) "
                "RETURN n.summary AS summary, n.uuid AS uuid",
                gid=graph_id, name_lower=username.lower(),
            )
            rec = r.single()
            if rec is None:
                return None, None
            return rec["summary"], rec["uuid"]

        with self._driver.session() as session:
            existing_summary, existing_uuid = self._call_with_retry(session.execute_read, _fetch_existing)

        # Accumulate comments: append `text` to the running summary unless it's already present.
        if existing_summary and text.strip() and text.strip() not in existing_summary:
            merged_summary = f"{existing_summary}\n\n• {text.strip()}"
        elif existing_summary:
            merged_summary = existing_summary
        else:
            merged_summary = text.strip()

        try:
            embedding = self._embedding.embed(f"{username}: {merged_summary}")
        except Exception as e:
            logger.warning(f"Commenter embedding failed, storing empty: {e}")
            embedding = []
        ...
        def _upsert(tx):
            result = tx.run(
                ...
                uuid=existing_uuid or new_uuid,
                name=username,
            ...
            record = result.single()
            return record["uuid"] if record else (existing_uuid or new_uuid)

        with self._driver.session() as session:
            return self._call_with_retry(session.execute_write, _upsert)
