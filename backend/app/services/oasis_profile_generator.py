...
logger = get_logger('neural_bridge.oasis_profile')
...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        self.simulation_requirement = (simulation_requirement or "").strip()
        self.neuro_prior = neuro_prior or {}
        self.neuro_modifiers = neuro_modifiers or {}
    ...
    def _has_sufficient_local_context(self, entity: EntityNode) -> bool:
        """Skip the expensive hybrid search when the entity node already carries
        enough attached edges/related-nodes to characterise the persona."""
        if not getattr(Config, "PERSONA_SKIP_HYBRID_SEARCH_IF_LOCAL", True):
            return False
        edges = len(entity.related_edges or [])
        nodes = len(entity.related_nodes or [])
        return (edges + nodes) >= 6 or bool((entity.summary or "").strip())
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            # Search edges (facts) — bounded to keep prompt compact.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 0. Simulation topic — always first so the LLM anchors every persona to it.
        if self.simulation_requirement:
            context_parts.append(
                "### Simulation Topic\nThis persona will participate in a public-perception "
                f"simulation about:\n{self.simulation_requirement}\n"
                "The persona's views, posting voice, and likely reactions must be consistent with "
                "their real comments AND plausible within this topic's context."
            )

        neuro_context = self._build_neuro_prior_context() if Config.NEURO_PRIOR_IN_PERSONA_PROMPTS else ""
        if neuro_context:
            context_parts.append(neuro_context)
        ...
        # 1b. Add structured data summary for metrics (agents can cite specific values)
        if entity.data_summary:
            data_lines = []
            ds = entity.data_summary
            if "latest_value" in ds:
                data_lines.append(f"**Latest Value**: {ds['latest_value']}")
            if "trend" in ds:
                data_lines.append(f"**Trend**: {ds['trend']}")
            if "pct_change" in ds:
                data_lines.append(f"**Change**: {ds['pct_change']:+.2f}%")
            if "min_value" in ds and "max_value" in ds:
                data_lines.append(f"**Range**: {ds['min_value']:.4g} to {ds['max_value']:.4g}")
            if "mean" in ds:
                data_lines.append(f"**Mean**: {ds['mean']:.4g}")
            if "std_dev" in ds:
                data_lines.append(f"**Volatility (Std Dev)**: {ds['std_dev']:.4g}")
            if data_lines:
                context_parts.append("### Concrete Data Context\nAgents should reference these values in discussion:\n" + "\n".join(data_lines))
        ...
        #    skipped when local context is already rich (bounded-state execution).
        if self._has_sufficient_local_context(entity):
            graph_results = {"facts": [], "node_summaries": [], "context": ""}
        else:
    ...
    def _build_neuro_prior_context(self) -> str:
        """Legacy research ablation; disabled by default to preserve identity."""
        if not self.neuro_prior and not self.neuro_modifiers:
            return ""
        prior = self.neuro_prior
        instruction = self.neuro_modifiers.get("persona_instruction", "")
        return (
            "### Behavioural Prior Context\n"
            "Use this only as a population-level behavioural bias. It should influence persona tendencies "
            "without overwriting the entity's identity or making every agent react the same way.\n"
            f"- salience: {prior.get('salience_score', 0.5)}\n"
            f"- threat sensitivity: {prior.get('threat_score', 0.5)}\n"
            f"- reward/hope sensitivity: {prior.get('reward_score', 0.5)}\n"
            f"- arousal: {prior.get('arousal_score', 0.5)}\n"
            f"- uncertainty: {prior.get('uncertainty_score', 0.5)}\n"
            f"- approach bias: {prior.get('approach_bias', 0.5)}\n"
            f"- avoidance bias: {prior.get('avoidance_bias', 0.5)}\n"
            f"- polarisation risk: {prior.get('polarisation_risk', 0.5)}\n"
            f"- virality pressure: {prior.get('virality_pressure', 0.5)}\n"
            f"Instruction: {instruction}\n"
            "Do not mention BOLD, amygdala activation, or brain reading. Describe reactions as expected "
            "salience, trust, threat, reward, uncertainty, approach, avoidance, and sharing behaviour."
        )
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                # Cacheable-prefix ordering: system prompt is identical across every
                # persona call in a run; only the user turn varies. LM Studio reuses
                # the KV cache for the stable prefix when it's first.
                response = self.client.chat.completions.create(
                    ...
                    max_tokens=getattr(Config, "PERSONA_MAX_TOKENS", 700),
                )

                content = response.choices[0].message.content or ""
                # Without response_format, Gemma may wrap JSON in ```json fences or add preamble. Strip both.
                ...
                try:
                    ...
                    anti_rep = "CRITICAL MANDATE: NEVER repeat what another agent has said. Do NOT regurgitate points. Be fiercely independent. If you agree, bring new data. If you disagree, attack their thesis aggressively."
                    if "rules" in result:
                        if isinstance(result["rules"], list):
                            result["rules"].append(anti_rep)
                        else:
                            result["rules"] = str(result["rules"]) + "\n" + anti_rep
                    else:
                        result["rules"] = [anti_rep]
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = (
            "You are a domain-adaptive social simulation persona architect. "
            "Create realistic personas for the specific simulation topic, whether it is politics, public health, "
            "finance, crisis communication, advertising, regulation, investor relations, consumer behaviour, "
            "or another public-facing stimulus. "
            "Personas must have distinct incentives, trust levels, risk sensitivity, identity anchors, information sources, "
            "and likely social-media behaviour. "
            "Use financial-market logic only when the simulation topic is actually financial. "
            "Return valid JSON only. All string values must contain no unescaped newlines."
        )
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        """Compact persona prompt — target-anchored, bounded context, short output."""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        ctx_limit = getattr(Config, "PERSONA_CONTEXT_CHAR_LIMIT", 1500)
        target_words = getattr(Config, "PERSONA_TARGET_WORDS", 250)
        context_str = context[:ctx_limit] if context else "No additional context"

        topic_line = ""
        if self.simulation_requirement:
            topic_line = (
                f"\nSimulation topic (anchor the persona to this): "
                f"{self.simulation_requirement[:600]}\n"
            )

        return f"""Generate a realistic social-media persona grounded in the entity and simulation topic. Return ONLY JSON.
{topic_line}
Entity: {entity_name} ({entity_type})
Summary: {entity_summary[:400] if entity_summary else ''}
...
JSON fields:
- bio: <=200 chars, first-person voice
- persona: ~{target_words} words, single paragraph, no newlines. Describe exactly who they are, what they care about, how they process trust/risk/status/identity, and how they might react to the simulation topic. Use trading strategy only for finance topics. DO NOT provide examples of exact posts or dictate their exact opinions.
- age: integer
- gender: "male" | "female"
- mbti: 4-letter code
- country: English name/code
- profession: short string
- interested_topics: array of 3-6 short strings tied to the simulation

Rules: 
1. Valid JSON only, no markdown, no unescaped newlines inside string values.
2. ABSOLUTE DATA STRICTNESS: Do not invent domain facts, news, metrics, statistics, credentials, affiliations, or events that are not present in the entity data or context.
3. Make the persona unique in its framework and strategy. You must NOT dictate the exact sentiment or exact phrases they will say. Let them decide organically during the simulation."""
    ...
    def _build_group_persona_prompt(
        ...
        """Compact institutional-account prompt — same bounded shape as individual."""
        ...
        if self.simulation_requirement:
            topic_line = (
                f"\nSimulation topic (anchor the account voice to this): "
                f"{self.simulation_requirement[:600]}\n"
            )

        return f"""Generate a realistic institutional social-media account profile grounded in the entity and simulation topic. Return ONLY JSON.
{topic_line}
Entity: {entity_name} ({entity_type})
Summary: {entity_summary[:400] if entity_summary else ''}
Attributes: {attrs_str}
...
JSON fields:
- bio: <=200 chars, official voice
- persona: ~{target_words} words, single paragraph, no newlines. Cover their official identity and institutional structural bias. DO NOT dictate how they should speak or provide likely statements.
- age: 30
- gender: "other"
- mbti: 4-letter code describing house style
- country: English name/code
- profession: short institutional descriptor
- interested_topics: array of 3-6 short strings tied to the simulation topic

Rules: 
1. Valid JSON only, no markdown, no unescaped newlines inside string values.
2. ABSOLUTE DATA STRICTNESS: Do not invent historical metrics or events not in the Context. Do NOT provide textual guidance on their exact outputs."""
    ...
    def generate_profiles_from_entities(
        ...
        graph_id: Optional[str] = None,
        parallel_count: int = 2,  # M2 Max 32GB budget
        ...
        parallel_count = min(parallel_count, 2)  # M2 Max 32GB budget
