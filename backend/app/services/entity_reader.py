...
logger = get_logger('neural_bridge.entity_reader')
...
class EntityNode:
    ...
    # Structured data summary for metrics (e.g., latest value, trend, range)
    data_summary: Optional[Dict[str, Any]] = field(default_factory=lambda: None)
...
class EntityReader:
    ...
    def filter_defined_entities(
        ...
        edge_refs_by_node = defaultdict(list)
        if enrich_with_edges:
            for edge in all_edges:
                edge_refs_by_node[edge["source_node_uuid"]].append(("outgoing", edge))
                edge_refs_by_node[edge["target_node_uuid"]].append(("incoming", edge))
        ...
        for node in all_nodes:
            ...
            data_summary = None
            try:
                # Try to parse data_summary_json if it exists
                data_summary_json = node.get("data_summary_json")
                if data_summary_json:
                    import json
                    data_summary = json.loads(data_summary_json)
            except Exception as e:
                logger.debug(f"Could not parse data_summary for {node.get('name')}: {e}")
            ...
            if enrich_with_edges:
                ...
                for direction, edge in edge_refs_by_node.get(node["uuid"], []):
