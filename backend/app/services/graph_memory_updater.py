...
logger = get_logger('neural_bridge.graph_memory_updater')
...
class GraphMemoryUpdater:
    ...
    # Larger batch = fewer NER calls = less graph-update overhead.
    BATCH_SIZE = getattr(Config, "GRAPH_MEMORY_BATCH_SIZE", 25)
    ...
    SEND_INTERVAL = getattr(Config, "GRAPH_MEMORY_SEND_INTERVAL", 0.2)
    MAX_RETRIES = 3
    RETRY_DELAY = 2

    # Actions that carry no textual signal for NER — skipped from graph writes
    # (they stay in the action log). These are pure interaction signals.
    LOW_INFO_ACTIONS = {
        "DO_NOTHING", "REFRESH", "TREND", "SEARCH_POSTS", "SEARCH_USER",
        "MUTE", "FOLLOW",
    }
    SKIP_LOW_INFO = getattr(Config, "GRAPH_MEMORY_SKIP_LOW_INFO_ACTIONS", True)
    ...
    def add_activity(self, activity: AgentActivity):
        """Add an agent activity to queue (filters low-info actions)."""
        if activity.action_type == "DO_NOTHING":
            self._skipped_count += 1
            return
        if self.SKIP_LOW_INFO and activity.action_type in self.LOW_INFO_ACTIONS:
            self._skipped_count += 1
            return
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        # Delta-oriented: dedupe identical episode lines within a batch so the
        # NER pass doesn't re-process repeated do-nothing-equivalent phrasings.
        seen = set()
        episode_texts = []
