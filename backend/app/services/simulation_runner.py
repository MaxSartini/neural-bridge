...
logger = get_logger('neural_bridge.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        cls,
        simulation_id: str,
        platform: str = "neural_bridge",  # neural_bridge / reddit / twitter / parallel
        ...
        Args:
            ...
            platform: Platform to run (neural_bridge/reddit/twitter/parallel)
        ...
        try:
            ...
            # Propagate performance knobs from Config into the subprocess so the
            # optimised behaviour is active by default (no manual wiring needed).
            env.setdefault('OASIS_ENV_SEMAPHORE', str(getattr(Config, 'OASIS_ENV_SEMAPHORE', 1)))
            env.setdefault('OASIS_MAX_ACTIVE_AGENTS_PER_ROUND',
                           str(getattr(Config, 'OASIS_MAX_ACTIVE_AGENTS_PER_ROUND', 8)))
            env.setdefault('OASIS_EMPTY_ROUND_SKIP_THRESHOLD',
                           str(getattr(Config, 'OASIS_EMPTY_ROUND_SKIP_THRESHOLD', 2)))
            env.setdefault('OASIS_SKIP_DEAD_HOURS',
                           'true' if getattr(Config, 'OASIS_SKIP_DEAD_HOURS', True) else 'false')
            env.setdefault('OASIS_SERIAL_PLATFORMS',
                           'true' if getattr(Config, 'OASIS_SERIAL_PLATFORMS', True) else 'false')
            env.setdefault(
                'OASIS_NEURO_PRIOR_IN_ROUND_PROMPTS',
                'true' if getattr(Config, 'OASIS_NEURO_PRIOR_IN_ROUND_PROMPTS', False) else 'false',
            )
            env.setdefault('OASIS_RANDOM_SEED', str(getattr(Config, 'OASIS_RANDOM_SEED', 33)))
            env.setdefault(
                'OASIS_LLM_TEMPERATURE',
                str(getattr(Config, 'OASIS_LLM_TEMPERATURE', 0.6)),
            )
            env.setdefault('OASIS_LLM_TOP_P', str(getattr(Config, 'OASIS_LLM_TOP_P', 0.9)))
