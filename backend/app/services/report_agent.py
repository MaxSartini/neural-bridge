...
logger = get_logger('neural_bridge.report_agent')
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'neural_bridge.report_agent',
            'neural_bridge.graph_tools',
        ]
...
class ReportAgent:
    ...
    def _load_json_file(self, path: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.warning(f"Failed to load report artifact {path}: {exc}")
        return None

    def _sample_action_log(self, path: str, limit: int = 8) -> Dict[str, Any]:
        stats = {"path": path, "total_actions": 0, "action_types": {}, "samples": []}
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    action_type = item.get("action_type") or item.get("type") or "unknown"
                    stats["action_types"][action_type] = stats["action_types"].get(action_type, 0) + 1
                    if len(stats["samples"]) < limit:
                        args = item.get("action_args") or item.get("args") or {}
                        content = ""
                        if isinstance(args, dict):
                            content = str(args.get("content") or args.get("text") or "")[:280]
                        stats["samples"].append({
                            "round": item.get("round_num") or item.get("round"),
    ...
    def _build_simulation_metadata_context(self) -> str:
        """Read local simulation artifacts so reports reflect the actual run."""
        sim_dir = self._get_simulation_dir()
        if not os.path.isdir(sim_dir):
            return "(Simulation artifact directory not found; rely on graph retrieval tools.)"

        config = self._load_json_file(os.path.join(sim_dir, "simulation_config.json")) or {}
        neuro_prior = self._load_json_file(os.path.join(sim_dir, "neuro_prior.json")) or config.get("neuro_prior") or {}
        neuro_modifiers = self._load_json_file(os.path.join(sim_dir, "neuro_prior_modifiers.json")) or config.get("neuro_modifiers") or {}

        compact = {
            "simulation_id": self.simulation_id,
            "artifact_dir": sim_dir,
            "time_config": config.get("time_config", {}),
            "enabled_platforms": config.get("enabled_platforms", {}),
            "agent_count": len(config.get("agent_configs", []) or []),
            "initial_posts": (config.get("event_config", {}) or {}).get("initial_posts", [])[:5],
            "neuro_prior": neuro_prior,
            "neuro_prior_modifiers": {
                "persona_instruction": (neuro_modifiers or {}).get("persona_instruction", ""),
                "activity_multiplier": (neuro_modifiers or {}).get("activity_multiplier"),
                "polarisation_multiplier": (neuro_modifiers or {}).get("polarisation_multiplier"),
                "virality_pressure": (neuro_prior or {}).get("virality_pressure") if isinstance(neuro_prior, dict) else None,
            },
            "action_log_summary": {
                "twitter": self._sample_action_log(os.path.join(sim_dir, "twitter", "actions.jsonl")),
                "reddit": self._sample_action_log(os.path.join(sim_dir, "reddit", "actions.jsonl")),
            },
            "reporting_caveat": (
                "Neuro-prior values are population-level behavioural calibration signals. "
                "Do not describe them as brain reading or individual diagnosis."
            ),
        }
    ...
    def plan_outline(
        ...
        simulation_metadata = self._build_simulation_metadata_context()
    ...
    def chat(
        ...
        try:
            ...
            if report and report.markdown_content:
                ...
                report_content = report.markdown_content[:6000]
                if len(report.markdown_content) > 6000:
