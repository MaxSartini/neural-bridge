"""Canonical Phase 5 adversarial correction + fix-plus retrain.
...
CORRECTION_SCHEMA_VERSION = "again_dense_2hz_phase5_adversarial_repair_fixplus_v1"
DEFAULT_CORRECTION_CONTROLS = (
    "ar_only_head",
    "pca_only_real",
    "pca_only_shuffled",
    "pca_only_random",
...
def row_dict_base_correction(
    ...
    row["schema_version"] = CORRECTION_SCHEMA_VERSION
    row.update(control_metadata(control))
    return row


def assemble_feature_blocks_correction(
    df: pd.DataFrame,
    dense_root: Path,
    phase4_root: Path,
    split: Any,
    ...
    *,
    ...
    if control_name == "ar_only_head":
        ...
        raise ValueError(f"Unsupported correction control: {control_name}")
...
def train_mlx_head_correction(
    ...
    if not best_checkpoint_path.exists():
        raise RuntimeError("Best checkpoint was not saved; refusing to score uncorrected Phase 5 model.")
...
def train_head_correction(backend: str, *args: Any, **kwargs: Any) -> Any:
    if backend != "mlx":
        raise ValueError("Correction runner requires MLX backend; CPU/MPS fallback is refused.")
    return train_mlx_head_correction(*args, **kwargs)


def metric_records_from_scores_correction(
    split: Any,
    feature: str,
    model: str,
    loss: str,
...
def promotion_gates_correction(summary: pd.DataFrame, phase4_root: Path, target_name: str) -> dict[str, Any]:
    gates: dict[str, Any] = {
    ...
    gates["recommendation"] = "correction_pass" if grouped_ok and blocked_ok else "correction_required"
...
def patch_base_module() -> None:
    ...
    base.PHASE5_SCHEMA_VERSION = CORRECTION_SCHEMA_VERSION
    ...
    base.assemble_feature_blocks = assemble_feature_blocks_correction
    base.row_dict_base = row_dict_base_correction
    base.train_mlx_head = train_mlx_head_correction
    base.train_head = train_head_correction
    base.metric_records_from_scores = metric_records_from_scores_correction
    base.promotion_gates = promotion_gates_correction
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.set_defaults(
        ...
        controls=",".join(DEFAULT_CORRECTION_CONTROLS),
...
def validate_phase4_feature_payload(args: argparse.Namespace) -> None:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            "Phase 4 feature payload is incomplete for this correction run. "
            f"Missing patterns under {features_dir}: {preview}{suffix}"
        )


def write_correction_artifacts(output_root: Path) -> None:
...
def run_correction(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not args.dry_run:
        write_correction_artifacts(output_root)
        summary = {
            "schema_version": CORRECTION_SCHEMA_VERSION,
            "created_at": datetime.now(timezone.utc).isoformat(),
            "output_root": str(output_root),
            "runtime_seconds_correction_wrapper": time.time() - start,
        ...
        write_json(output_root / "correction_fixplus_summary.json", summary)
    return result
