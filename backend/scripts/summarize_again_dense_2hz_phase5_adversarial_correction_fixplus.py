"""Summarize canonical Phase 5 adversarial correction + fix-plus outputs."""
...
CORRECTION_SCHEMA_VERSION = "again_dense_2hz_phase5_adversarial_correction_fixplus_summary_v1"
...
def summarize(output_root: Path, reports_dir: Path) -> dict[str, Any]:
    ...
    if not summary_path.exists() or not fold_path.exists():
        raise FileNotFoundError(f"Missing correction metrics under {metrics}")
    ...
    report = reports_dir / f"again_dense_2hz_phase5_adversarial_correction_fixplus_summary_{stamp}.md"
    lines = [
        "# AGAIN Dense 2Hz Phase 5 Adversarial Correction Fix-Plus Summary",
        "",
