...
def test_live_state_authorizes_only_phase02_after_fresh_phase01() -> None:
    ...
    assert "Authorized action: run and verify the corrected end-to-end" in current
    assert "executor-backtest-registration.json" in current
    assert "Only the corrected end-to-end aggregation executor backtest" in current
    assert "selected-rescue-executor.json" in current
    assert "14,465" in current
    assert "113,392" in current
    assert "82,566" in current
    assert "30,826" in current
